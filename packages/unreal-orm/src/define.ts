	...
	class DynamicModelBase extends BaseTable<TableData> {
		...
		static async create<T extends typeof DynamicModelBase>(
			...
			if (!createdRecords || createdRecords.length === 0) {
				...
				throw new Error(
					`Failed to create record in ${this.getTableName()}. No record returned.`,
				);
			}
		...
		constructor(data: TableData & Record<string, unknown>) {
			...
			function hydrateValue(
				...
				if (
					...
					return new RelatedModel(
						value as InferTableDataFromFields<typeof RelatedModel._fields>,
					);
				}
				...
				// Hydrate nested object fields recursively
				if (
					...
					for (const key in fieldDef.objectSchema) {
						if (
							Object.prototype.hasOwnProperty.call(fieldDef.objectSchema, key)
						) {
					...
					// Assign any dynamic/extra keys not in the schema as-is
			...
			// Hydrate all fields using the helper
			for (const key in data) {
				...
				if (!fieldDef) {
					this.$dynamic[key] = rawValue;
					continue;
				}

				(this as Record<string, unknown>)[key] = hydrateValue(
					rawValue,
					fieldDef,
				);
			}
		...
		async update(db: Surreal, data: Partial<CreateInputData>): Promise<void> {
			...
			for (const key in data) {
				if (
					...
					key in DynamicModelBase._fields
				) {
					(this as { [key: string]: unknown })[key] =
						data[key as keyof CreateInputData];
				}
			}
		}
		...
		static async select<
			T extends typeof DynamicModelBase,
			O extends SelectQueryOptions<InferTableDataFromFields<T["_fields"]>> & {
				groupBy: string[];
			},
		>(this: T, db: Surreal, options: O): Promise<Record<string, unknown>[]>;
		...
		>(
			...
			O["only"] extends true
				? Partial<InferTableDataFromFields<T["_fields"]>> | undefined
				: Partial<InferTableDataFromFields<T["_fields"]>>[]
		>;
		...
		>(
			...
			O["only"] extends true ? InstanceType<T> | undefined : InstanceType<T>[]
		>;
		...
		static async select<T extends typeof DynamicModelBase>(
			...
			// Return type is 'unknown' as this implements multiple differently-typed overloads.
			const opts = options || {};
			...
			const instances = recordArray.map(
				(r) => new this(r) as ModelInstanceType,
			);
...
const Table = {
	...
	>(
		...
		ModelInstance<
			InferTableDataFromFields<RelationTableFields<TIn, TOut, TOther>>
		>,
