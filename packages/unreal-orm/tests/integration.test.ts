...
import { applySchema, generateTableSchemaQl } from "../src/schemaGenerator"; // Use relative import
...
describe("FLEXIBLE object/custom field SurrealQL generation", () => {
  ...
  test("should generate SurrealQL with FLEXIBLE TYPE object for flexible fields", () => {
    const ddl = generateTableSchemaQl(FlexibleModel);
    console.log(ddl);
    ...
    // Should *not* emit FLEXIBLE for regular object field
    expect(ddl).toContain(
      "DEFINE FIELD regular ON TABLE flexible_model TYPE object"
    );
    expect(ddl).not.toContain(
      "DEFINE FIELD regular ON TABLE flexible_model FLEXIBLE TYPE object"
    );
  });

  test("should apply schema, insert, and select flexible fields without error", async () => {
    // Use a separate in-memory Surreal instance
    const db = new Surreal({ engines: surrealdbNodeEngines() });
    await db.connect("mem://");
    await db.use({ namespace: "test_flexible", database: "test_flexible" });
    await applySchema(db, [FlexibleModel], "IF NOT EXISTS");

    // Insert a record with flexible fields
    const data = {
      meta: { foo: "bar" },
      data: { baz: 123, arr: [1, 2, 3], nested: { x: true } },
      regular: { bar: "baz" },
    };
    const inserted = await FlexibleModel.create(db, data);
    ...
    expect(inserted.regular).toEqual({ bar: "baz" });

    // Fetch the record back
    const fetched = await FlexibleModel.select(db, {
      from: inserted.id,
      only: true,
    });
    console.log(fetched);
    expect(fetched).toBeTruthy();
    expect(fetched?.meta).toEqual({ foo: "bar" });
    expect(fetched?.data).toEqual({
      baz: 123,
      arr: [1, 2, 3],
      nested: { x: true },
    });
    expect(fetched?.regular).toEqual({ bar: "baz" });

    await db.delete("flexible_model");
    await db.close();
  });
});
...
class User extends Table.normal({
  fields: {
    ...
    settings: Field.object(
      {
        profile_visibility: Field.string({
          default: '"public"',
          assert: '$value INSIDE ["public", "private", "friends"]',
        }),
      },
      { default: "{}" }
    ),
    optionalBio: Field.option(
      Field.string({ assert: "string::len($value) <= 500" })
    ),
...
class Post extends Table.normal({
  ...
  getAuthorName(): string | undefined {
    ...
    if (
      authorInstance &&
      typeof authorInstance === "object" &&
      "name" in authorInstance &&
      typeof authorInstance.name === "string"
    ) {
...
describe("ORM Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const post1 = (await Post.create(db, {
      title: "First Post",
      content: "...",
      author: user1Id,
      published: true,
  ...
  test("should fetch all users and instantiate them correctly", async () => {
    ...
    if (!(user1 instanceof User)) {
      throw new Error(
        "Test setup error: user1 is not an instance of User as expected."
      );
    }
  ...
  test("should select specific fields and return raw objects", async () => {
    ...
    expect(usersNameEmail[0]).toEqual({
      name: "Alice",
      email: "alice@example.com",
    });
  });
  ...
  test("should select users with a WHERE clause", async () => {
    const usersOver20 = await User.select(db, {
      where: "age > $minAge",
      vars: { minAge: 20 },
    });
    ...
    if (!user) {
      throw new Error(
        "Expected to select at least one user, but the array was empty."
      );
    }
  ...
  test("should select users with GROUP BY and return raw data", async () => {
    ...
    const activeGroup = (usersByStatusRaw as GroupedUser[]).find(
      (g) => g.status === "active"
    );
    if (!activeGroup) {
      throw new Error(
        "Expected to find the active group, but it was not found."
      );
    }
  ...
  test("should fetch a related record and hydrate it into a model instance", async () => {
    ...
    if (!(author instanceof User)) {
      throw new Error(
        "The author property was not hydrated into a User instance."
      );
    }
  ...
  test("should fetch a user with an array of related posts and hydrate them", async () => {
    ...
    if (!fetchedPost1 || !fetchedPost2) {
      throw new Error(
        "Expected to find two posts, but they were not found in the array."
      );
    }
  ...
  test("should fail to create a user with an invalid email", async () => {
    ...
    expect(
      await User.create(db, invalidUserData).catch(() => undefined)
    ).toBeUndefined();
  });
